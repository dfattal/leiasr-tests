...
// Global Variables.
const char*               g_windowTitle               = "LeiaSR Parallax Toggle Demo (D3D11)"; // Title of the window
const char*               g_windowClass               = g_windowTitle;                          // Class name for the window
SR::SRContext*            g_srContext                 = nullptr;                                // SR context
SR::IDisplayManager*      g_displayManager            = nullptr;                                // Display manager (modern API)
SR::IDisplay*             g_display                   = nullptr;                                // Display interface (modern API)
...
vec3f                     g_defaultViewingPosition    = vec3f(0.0f, 0.0f, 600.0f);             // Default viewing position from display
...
bool                      g_parallaxEnabled           = true;                                   // Parallax toggle state (true = normal, false = centered at default view position)
...
bool CreateSRContext(double maxTime)
{
    ...
    // Get display manager (modern API, no lazy binding fallback).
    if (g_srContext != nullptr)
    {
        ...
        {
            OnError("Failed to get DisplayManager. This example requires runtime version 1.34.8-RC1 or later.");
            return false;
        }
    }
...
bool InitializeLeiaSR(double maxTime)
{
    ...
    // Get default viewing position (NEW API).
    float x_mm, y_mm, z_mm;
    g_display->getDefaultViewingPosition(x_mm, y_mm, z_mm);
    g_defaultViewingPosition = vec3f(x_mm, y_mm, z_mm);
    std::cout << "Default viewing position: (" << x_mm << ", " << y_mm << ", " << z_mm << ") mm" << std::endl;
...
void UpdateWindowTitle(HWND hWnd, double curTime)
{
    ...
    if (curTime - prevTime > 0.25)
    {
        ...
        sprintf(newWindowTitle, "%s (%.1f FPS) - Parallax: %s",
            g_windowTitle, fps, g_parallaxEnabled ? "ON" : "OFF");
...
void Render()
{
    ...
    // Apply parallax toggle logic.
    if (!g_parallaxEnabled)
    {
        // Calculate mid-eye point.
        vec3f midEye = (leftEye + rightEye) * 0.5f;

        // Calculate translation to move mid-eye to default viewing position.
        vec3f translation = g_defaultViewingPosition - midEye;

        // Apply rigid translation to both eyes.
    ...
    // Update window title with FPS and parallax state.
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        switch (wParam)
        {
            ...
            case 'P':
            {
                g_parallaxEnabled = !g_parallaxEnabled;
                std::cout << "Parallax " << (g_parallaxEnabled ? "ENABLED" : "DISABLED") << std::endl;
                return 0; // Message handled
            }

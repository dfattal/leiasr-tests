...
public:
     ...
     */
    static Window2* create(SRContext &context, SR_HWND window);
    ...
    /**
     * \brief Get the screen space coordinates of the window. Assumes fullscreen when window is invalid
     * \param monitorRect The reference screen in desktop coordinates
     * \return The position and size of the window in screen space coordinates
     */
    static void getScreenRect(const std::shared_ptr<SR::Window2>& window, const SR_recti& monitorRect, int screenWidth, int screenHeight, int& rectX, int& rectY, int& rectW, int& rectH);
};

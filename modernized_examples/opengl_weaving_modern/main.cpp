...
bool CreateSRContext(double maxTime)
{
    ...
    if (g_srContext)
    {
        // Use modern display helper with built-in wait functionality
        SR::Helper::DisplayAccess displayAccess(*g_srContext);
...
bool InitializeLeiaSR(double maxTime)
{
    ...
    // Get recommended view texture size using modern display helper
    SR::Helper::DisplayAccess displayAccess(*g_srContext);
    g_viewTextureWidth  = displayAccess.getRecommendedViewsTextureWidth();
    g_viewTextureHeight = displayAccess.getRecommendedViewsTextureHeight();
